...
with open("calculatorprint_github.py", "w") as f:
    f.write("import os\n")
    f.write("""
...
f.write("num2 = int(input('Please choose your second number: '))")\n""")
    ...
    for display, operator in [("+", "+"), ("-", "-"), ("+", "*")]:
        f.write("""
i = 0
while i < 100:
    j = 0
    while j < 100:
        f.write("if num1 == %d and sign == '{1}' and num2 == %d :" %(i , j))
        f.write("\\tprint(\\" %d {0} %d = %d \\"" %(i, j, i{1}j))
        j += 1
    i += 1\n""".format(display, operator))
